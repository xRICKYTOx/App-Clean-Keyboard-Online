...
def InstallApp(path):
    ...
    class InstallApp():
        ...
        def moved_int_path(self):
            ...
            path_exe_desktop = f'{path_complet}/CleanKeyboard.exe'

            move_path_lnk_desktop = f'CleanKeyboard'

            with winshell.shortcut(move_path_lnk_desktop) as acceso_directo_desktop:
                acceso_directo_desktop.path = path_exe_desktop
                acceso_directo_desktop.icon_location = (path_exe_desktop, 0)

            link1 = 'CleanKeyboard.lnk'
            desktop = f'C:/Users/{user_pc}/OneDrive/Escritorio/'

            shutil.move(link1, desktop)

            # Moved ==> panel apps

            move_path_lnk_apps = f'CleanKeyboard'

            with winshell.shortcut(move_path_lnk_apps) as acceso_directo_desktop:
                acceso_directo_desktop.path = path_exe_desktop
                acceso_directo_desktop.icon_location = (path_exe_desktop, 0)

            link2 = 'CleanKeyboard.lnk'
            apps_panel = f'C:\ProgramData\Microsoft\Windows\Start Menu\Programs'

            shutil.move(link2, apps_panel)

            self.win.destroy()
            ExitAppSelect()


# "C:\Users\rikyt\OneDrive\Escritorio\AndroidStudio.lnk"
    InstallApp()
